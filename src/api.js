...
import { makeTimeLogApi } from './api/timeLogs.js';
...
/* ------------------------------------------------------------------ */
/* Time logs — SHARED with the Chrome extension (src/api/timeLogs.js). */
/* Bound here to the web app's client; the extension binds its own.   */
/* Requires fixes23.sql. Reads degrade to [] until it's deployed.      */
/* ------------------------------------------------------------------ */
const timeLogApi = makeTimeLogApi(supabase);
export const {
  fetchTimeLogsForProject, fetchMyTimeLogsForProject, fetchMyOpenTimeLog,
  createTimeLog, deleteTimeLog,
  startTimeLog, pauseTimeLog, resumeTimeLog, stopTimeLog, discardTimeLog,
} = timeLogApi;
export { mapTimeLog, elapsedMs } from './api/timeLogs.js';
